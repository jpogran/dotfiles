...
$files | % {
  ...
  switch($name){
    ...
    'vscode.settings.json'{ $linkDir = "$($HOME)/Library/Application Support/Code/User/settings.json" }
    'profile.ps1'{ $linkDir = '~/.config/powershell/profile.ps1' }
    default{ $linkDir =  "$($HOME)/$($name)" }
  }
  if($linkDir){
